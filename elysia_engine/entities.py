...
from .thermodynamics import ThermalState, MatterState, Crystallizer
...
class Entity:
    ...
    def step(self, world: WorldLike, dt: float = 1.0, chronos: Optional[ChronosMaster] = None) -> None:
        ...
        """
        # 0. Check Thermodynamics (Ice Star Check)
        if self.thermal:
            # Cooling
            self.thermal.cool_down(dt)
            if self.thermal.state == MatterState.CRYSTAL:
                # Frozen entities do not compute physics or logic.
    ...
    def evolve_to_field(self) -> Optional[Any]:
        """
        Placeholder for Fractal Evolution (Point -> Line -> Plane).
        To be implemented by FractalEvolver.
        """
        return None
